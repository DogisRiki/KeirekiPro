...
4. [Google OAuth認証情報の取得](#4-google-oauth認証情報の取得)
5. [GitHub OAuth認証情報の取得](#5-github-oauth認証情報の取得)
6. [JWT署名鍵の生成](#6-jwt署名鍵の生成)
7. [GitHub Secretsへの機密情報登録](#7-github-secretsへの機密情報登録)
8. [Terraform実行後の作業](#8-terraform実行後の作業)
...
3. 以下を入力:
   ...
   - 承認済みのリダイレクトURI: `https://app.keirekipro.click/api/auth/callback/google`
4. 「作成」をクリック
...
3. 以下を入力:
   ...
   - Authorization callback URL: `https://app.keirekipro.click/api/auth/callback/github`
...
## 7. GitHub Secretsへの機密情報登録

Terraform実行時に使用する機密情報をGitHub Secretsに登録する。

### 7.1 GitHub Secretsにアクセス
...
| Name | Secret（値） |
|------|-------------|
| `TF_VAR_jwt_secret` | 手順6で生成したJWT署名鍵 |
| `TF_VAR_google_oauth_client_id` | 手順4で取得したGoogleクライアントID |
| `TF_VAR_google_oauth_client_secret` | 手順4で取得したGoogleクライアントシークレット |
| `TF_VAR_github_oauth_client_id` | 手順5で取得したGitHubクライアントID |
| `TF_VAR_github_oauth_client_secret` | 手順5で取得したGitHubクライアントシークレット |
