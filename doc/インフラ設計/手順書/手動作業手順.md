  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          ...
          "token.actions.githubusercontent.com:repository": "<GitHubユーザー名>/KeirekiPro",
...
取得できなくなる。ワークフローから `environment: production` の行を消した実行は
environmentクレームを持たないため、引き受けに失敗する。
