...
| log_timezone | Asia/Tokyo | ログに記録されるタイムゾーン。timezoneと合わせることでログの時刻が日本時間になる |
...
| 項目 | 設定値 |
|------|--------|
| マスターユーザー名 | Secrets Managerで管理 |
| マスターパスワード | Secrets Managerで管理 |
| IAMデータベース認証 | 無効 |
...
| 項目 | 設定値 |
|------|--------|
| ユーザー名 | Secrets Managerで管理（keirekipro/rds） |
| パスワード | Secrets Managerで管理（keirekipro/rds） |
| 権限 | keireki_proデータベースへの全権限 |

### 10.3 テーブル構築・マスタデータ挿入
...
## 11. Secrets Manager連携

| シークレット名 | 用途 |
|---------------|------|
| keirekipro/rds | RDS接続情報（host, port, database, username, password） |
