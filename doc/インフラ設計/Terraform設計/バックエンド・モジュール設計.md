...
| ecs | ECSクラスター、タスク定義、サービス | vpc, iam, alb, secrets-manager |
| cloudfront | CloudFrontディストリビューション、OAC | s3, acm, secrets-manager |
| waf | WAF Web ACL、ルール | cloudfront |
| route53 | Route 53レコード | cloudfront, alb |
| ses | SES設定セット | route53 |
| cloudwatch | ロググループ、アラーム、SNSトピック、ダッシュボード | ecs, alb, rds, elasticache |

※ GitHub Actions用IAMロール・OIDCプロバイダーはTerraformでは管理せず、AWSコンソールから手動で作成・管理する。
※ ECRリポジトリはルートモジュール（main.tf）で直接定義する。
...
| リソース種別 | リソース名 |
|-------------|-----------|
| aws_iam_role | keirekipro-ecs-task-execution-role, keirekipro-ecs-task-role |
| aws_iam_policy | keirekipro-ecs-secrets-policy, keirekipro-app-secrets-policy, keirekipro-s3-policy, keirekipro-ses-policy |
| aws_iam_role_policy_attachment | 各ロールへのポリシーアタッチ |

※ GitHub Actions用IAMロール・OIDCプロバイダーはTerraformでは管理しない。
...
| リソース種別 | リソース名 |
|-------------|-----------|
| aws_acm_certificate | app.keirekipro.click（us-east-1）, app.keirekipro.click（ap-northeast-1）, api.keirekipro.click（ap-northeast-1） |
...
| aws_lb_listener_certificate | app.keirekipro.click用追加証明書 |
...
| リソース種別 | リソース名 |
|-------------|-----------|
| aws_route53_record | app.keirekipro.click（A、AAAA）、api.keirekipro.click（A）、ACM検証レコード |
...
| alb_certificate_arn | ALB用証明書ARN（ap-northeast-1・app） |
| alb_api_certificate_arn | ALB用API証明書ARN（ap-northeast-1・api） |
...
| security_group_id | string | セキュリティグループID |
| certificate_arn | string | ACM証明書ARN（app） |
| api_certificate_arn | string | ACM証明書ARN（api） |
...
| alb_arn_suffix | ALB ARNサフィックス |
| alb_zone_id | ALBホストゾーンID |
| target_group_arn | ターゲットグループARN |
| target_group_arn_suffix | ターゲットグループARNサフィックス |
...
| ecr_repository_url | string | ECRリポジトリURL |
...
| api_domain_name | string | APIサブドメイン名 |
| origin_verify_header_value | string | Origin Verifyヘッダー値 |
...
| alb_zone_id | string | ALBホストゾーンID |
