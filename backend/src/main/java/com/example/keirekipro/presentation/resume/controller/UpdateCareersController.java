...
public class UpdateCareersController {

    private final UpdateCareersUseCase updateCareersUseCase;
    ...
    public ResumeInfoResponse handle(
            ...
            @Valid @RequestBody UpdateCareersRequest request) {

        UUID userId = UUID.fromString(currentUserFacade.getUserId());
        UUID resumeUuid = UUID.fromString(resumeId);

        return ResumeInfoResponse.convertToResponse(
                updateCareersUseCase.execute(userId, resumeUuid, request));
    }
}
