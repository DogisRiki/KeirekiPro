...
public class UpdateCertificationsController {

    private final UpdateCertificationsUseCase updateCertificationsUseCase;
    ...
    public ResumeInfoResponse handle(
            ...
            @Valid @RequestBody UpdateCertificationsRequest request) {

        UUID userId = UUID.fromString(currentUserFacade.getUserId());
        UUID resumeUuid = UUID.fromString(resumeId);

        return ResumeInfoResponse.convertToResponse(
                updateCertificationsUseCase.execute(userId, resumeUuid, request));
    }
}
