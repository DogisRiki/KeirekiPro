...
public class UpdateProjectsController {
    ...
    private final UpdateProjectsUseCase updateProjectsUseCase;
    ...
    public ResumeInfoResponse handle(
            ...
            @Valid @RequestBody UpdateProjectsRequest request) {

        UUID userId = UUID.fromString(currentUserFacade.getUserId());
        UUID resumeUuid = UUID.fromString(resumeId);

        return ResumeInfoResponse.convertToResponse(
                updateProjectsUseCase.execute(userId, resumeUuid, request));
    }
}
