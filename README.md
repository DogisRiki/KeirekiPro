...
frontend(npm)はバージョン更新の対象に含めていません。Dependabotがサポートするpnpmは10までで、このリポジトリが指定しているpnpm 11に未対応のためです。対応が入るまで、frontendのバージョン更新は通常のプルリクエストで行います。脆弱性の修正は設定ファイルに依存せずアラート起点で作成されるため、frontendでも作成されます。
