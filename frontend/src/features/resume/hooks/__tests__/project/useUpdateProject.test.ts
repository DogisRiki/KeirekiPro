...
describe("useUpdateProject", () => {
    ...
    it("成功時はエントリエラーをクリアし、projectsをマージしてストア更新・dirty解除・通知が実行されること", async () => {
        ...
        expect(useResumeStore.getState().clearEntryErrors).toHaveBeenNthCalledWith(1, projectId);
        expect(useResumeStore.getState().clearEntryErrors).toHaveBeenNthCalledWith(2, projectId);
    ...
    it("失敗時はエラーレスポンスのerrorsが存在する場合に、該当エントリへエラーが設定されること", async () => {
        ...
        expect(useResumeStore.getState().clearEntryErrors).toHaveBeenCalledWith(projectId);

        expect(useResumeStore.getState().setEntryErrors).toHaveBeenCalledWith(projectId, mockErrorResponse.errors);
    ...
    it("プロジェクト不存在404の場合、空セクションが省略された詳細レスポンスでストアを同期すること", async () => {
        ...
        act(() => {
            result.current.mutate({ projectId, payload: {} as any });
        });
