...
import { useResumeStore, useUpdateSocialLinks } from "@/features/resume";
...
describe("useUpdateSocialLinks", () => {
    ...
    const mockResume: Resume = {
        ...
        certifications: [],
        portfolios: [],
        socialLinks: [{ id: "social-1", name: "GitHub", link: "https://github.com/example" }],
        selfPromotions: [],
    };
    ...
    it("成功時はエラーストアをクリアし、ストア更新と成功通知が実行されること", async () => {
        ...
        const { result } = renderHook(() => useUpdateSocialLinks("resume-1"), { wrapper });

        act(() => {
            result.current.mutate({
                socialLinks: [{ id: null, name: "GitHub", link: "https://github.com/example" }],
            });
        });
        ...
        expect(useResumeStore.getState().updateResume).toHaveBeenCalledWith({
            socialLinks: mockResume.socialLinks,
            updatedAt: mockResume.updatedAt,
        });
        expect(useResumeStore.getState().clearDirtyEntryIds).toHaveBeenCalledWith(["social-1"]);
