...
import { useResumeStore, useUpdateCareers } from "@/features/resume";
...
describe("useUpdateCareers", () => {
    ...
    beforeEach(() => {
        ...
        useResumeStore.getState().setResume({
            ...mockResume,
            careers: [{ id: "career-1", companyName: "更新前会社", startDate: "2020-04", endDate: null, active: true }],
        });
    ...
    it("成功時はエラーストアをクリアし、ストア更新と成功通知が実行されること", async () => {
        ...
        const { result } = renderHook(() => useUpdateCareers("resume-1"), { wrapper });

        act(() => {
            result.current.mutate({
                careers: [{ id: null, companyName: "更新後会社", startDate: "2020-04", endDate: null, isActive: true }],
            });
        });
        ...
        expect(useResumeStore.getState().updateResume).toHaveBeenCalledWith({
            careers: mockResume.careers,
            updatedAt: mockResume.updatedAt,
        });
        expect(useResumeStore.getState().clearDirtyEntryIds).toHaveBeenCalledWith(["career-1"]);
